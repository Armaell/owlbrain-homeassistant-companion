...
async def test_load_existing_data(store):
	store._store.async_load.return_value = {
		"devices": [{"namespace": "ns", "device_id": "dev1", "value": "A"}],
		"entities": [{"namespace": "ns", "entity_id": "ent1", "value": "B"}],
	}
...
async def test_set_and_get_device(store):
	...
	dev = DeviceModel.from_dict(
		{"namespace": "ns", "device_id": "dev1", "metadata": "D"}
	)
...
async def test_set_and_get_entity(store):
	...
	ent = EntityModel.from_dict(
		{"namespace": "ns", "entity_id": "ent1", "data": "D"}
	)
