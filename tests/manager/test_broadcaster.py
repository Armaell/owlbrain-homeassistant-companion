...
def test_remove_removes_connection_and_triggers_refresh_when_last(
	broadcaster, mock_manager
):
	# Arrange
	namespace = "ns1"
	conn = Mock()
...
def test_remove_does_not_trigger_refresh_if_other_connections_remain(
	broadcaster, mock_manager
):
	# Arrange
	namespace = "ns1"
...
@pytest.mark.parametrize("existing, expected", [([], False), ([Mock()], True)])
...
@pytest.mark.asyncio
async def test_broadcast_entity_action_constructs_and_broadcasts_message(
	broadcaster,
):
	# Arrange
	namespace = "ns1"
	...
	# Act
	await broadcaster.broadcast_entity_action(
		namespace, entity_id, action, data
	)
