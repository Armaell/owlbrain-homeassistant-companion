...
async def test_update_metadata(hass):
	helper = EntityTestHelper(OwlBrainSensorEntity, hass=hass)

	await helper.run_metadata_matrix(
		[
			("unit", "unit", None, {"native_unit_of_measurement": "unit"}),
			("device_class", "battery", None, {"device_class": "battery"}),
			(
				"device_class",
				"invalid",
