...
async def test_update_metadata(hass):
	helper = EntityTestHelper(OwlBrainSwitchEntity, hass=hass)

	await helper.run_metadata_matrix(
		[("device_class", "outlet", None, {"device_class": "outlet"})]
	)
...
async def test_actions(hass):
	...
	await helper.run_action_matrix(
		[
			("async_turn_on", None, "turn_on", None),
			("async_turn_off", None, "turn_off", None),
		]
	)
