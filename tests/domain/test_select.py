...
async def test_update_metadata(hass):
	helper = EntityTestHelper(OwlBrainSelectEntity, hass=hass)

	await helper.run_metadata_matrix(
		[
			("options", ["a", "b"], None, {"options": ["a", "b"]}),
			("options", "invalid", OwlInvalidValueError, {"options": []}),
		]
	)
...
async def test_update_data(hass):
	helper = EntityTestHelper(
		OwlBrainSelectEntity, hass=hass, metadata={"options": ["a", "b"]}
	)

	await helper.run_data_matrix(
		[
			("state", "a", None, {"current_option": "a"}),
			("state", "c", OwlInvalidValueError, {"current_option": None}),
		]
	)
...
async def test_actions(hass):
	...
	await helper.run_action_matrix(
		[("async_select_option", ("b",), "select", {"state": "b"})]
	)
