...
from custom_components.owlbrain.domain.binary_sensor import (
...
async def test_update_metadata(hass):
	helper = EntityTestHelper(OwlBrainBinarySensorEntity, hass=hass)

	await helper.run_metadata_matrix(
		[("device_class", "foo", None, {"device_class": "foo"})]
	)
...
async def test_update_data(hass):
	...
	await helper.run_data_matrix(
		[
			("state", "on", None, {"is_on": True}),
			("state", "off", None, {"is_on": False}),
			("state", "invalid", OwlInvalidValueError, {"is_on": None}),
		]
	)
...
async def test_light_platform_registration(hass):
	helper = EntityTestHelper(
		entity_cls=None, hass=hass, domain="binary_sensor"
	)
