...
class OwlBrainStore:
    ...
    def sync_get_device(self, namespace: str, device_id: str) -> DeviceModel | None:
        if self._devices is None:
            return None
        raw = self._devices.get((namespace, device_id))
        return DeviceModel.from_dict(raw) if raw else None

    async def get_device(self, namespace: str, device_id: str) -> DeviceModel | None:
    ...
    async def get_devices(self, namespace = None) -> Dict[tuple, DeviceModel]:
        ...
        return {
            ...
            for key, raw in self._devices.items() if namespace is None or key[0] == namespace
        }

    async def get_entity(self, namespace: str, entity_id: str) -> EntityModel | None:
        await self._ensure_loaded()
    ...
    async def get_entities(self, namespace = None) -> Dict[tuple, EntityModel]:
        ...
        return {
            ...
            for key, raw in self._entities.items() if namespace is None or key[0] == namespace
        }
